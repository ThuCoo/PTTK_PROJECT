...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, data: await ThanhToanBUS.getAll(search, trang_thai) });
  } catch (err: any) {
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    res.json({ success: true, data: await ThanhToanBUS.getById(parseInt(req.params.id)) });
  } catch (err: any) {
...
router.post('/:id/pay', async (req: Request, res: Response) => {
  try {
    await ThanhToanBUS.markPaid(parseInt(req.params.id), req.body.phuong_thuc);
