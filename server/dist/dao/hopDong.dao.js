...
exports.recordCheckoutTime = recordCheckoutTime;
...
async function getById(maHopDong) {
    const result = await (0, db_1.query)(`SELECT hd.*, k.ho_ten as ten_khach, k.sdt as phone_khach
     FROM hop_dong hd
     LEFT JOIN khach_hang k ON hd.ma_khach_hang = k.ma_khach_hang
     WHERE hd.ma_hop_dong = $1`, [String(maHopDong)]);
    return result.rows[0] || null;
}
async function create(data) {
    const result = await (0, db_1.query)(`INSERT INTO hop_dong
...
async function sign(maHopDong) {
    await (0, db_1.query)(`UPDATE hop_dong SET trang_thai='Đang hiệu lực', ngay_ky=CURRENT_DATE WHERE ma_hop_dong=$1`, [String(maHopDong)]);
}
async function terminate(maHopDong) {
    await (0, db_1.query)(`UPDATE hop_dong SET trang_thai='Đã kết thúc' WHERE ma_hop_dong=$1`, [String(maHopDong)]);
}
async function finalize(id) {
    await (0, db_1.query)(`UPDATE hop_dong SET trang_thai='Đã thanh lý' WHERE ma_hop_dong=$1`, [id]);
}
async function recordCheckoutTime(id, checkoutTime) {
    await (0, db_1.query)(`UPDATE hop_dong SET ngay_tra_thuc_te=$1 WHERE ma_hop_dong=$2`, [
        checkoutTime,
        id,
    ]);
}
async function getByStatus(trangThai) {
    const result = await (0, db_1.query)(`SELECT hd.*, k.ho_ten as ten_khach, k.sdt as phone_khach
     FROM hop_dong hd
     LEFT JOIN khach_hang k ON hd.ma_khach_hang = k.ma_khach_hang
     WHERE hd.trang_thai = $1
     ORDER BY hd.created_at DESC`, [trangThai]);
    return result.rows;
}
async function addGroupMembers(hopDongId, members) {
    for (const m of members) {
        await (0, db_1.query)(`INSERT INTO thanh_vien_nhom (hop_dong_id, ho_ten, cccd, phone, ngay_sinh, dia_chi_thuong_tru)
       VALUES ($1,$2,$3,$4,$5,$6)`, [hopDongId, m.ho_ten, m.cccd, m.phone, m.ngay_sinh, m.dia_chi_thuong_tru]);
    }
}
