...
//   try {
//     const data = await DatCocBUS.getById(parseInt(req.params.id));
//     res.json({ success: true, data });
//   } catch (err: any) {
//     res.status(404).json({ success: false, error: err.message });
//   }
...
//   try {
//     const result = await DatCocBUS.getProofImage(parseInt(req.params.id));
//     if (!result) { res.status(404).json({ success: false, error: 'Không có ảnh chứng từ' }); return; }
//     // Return as data URL
//     res.json({ success: true, data: { dataUrl: `data:${result.mimeType};base64,${result.base64}` } });
//   } catch (err: any) {
//     res.status(500).json({ success: false, error: err.message });
//   }
// });
...
// router.post('/:id/upload', upload.single('proof'), async (req: Request, res: Response) => {
//   try {
//     if (!req.file) { res.status(400).json({ success: false, error: 'Chưa chọn file ảnh' }); return; }
...
//     res.json({ success: true, message: 'Đã tải lên chứng từ thành công. Quản lý sẽ xác nhận sớm.' });
//   } catch (err: any) {
//     res.status(400).json({ success: false, error: err.message });
//   }
// });
...
//     await DatCocBUS.confirm(parseInt(req.params.id), nguoiXacNhan);
...
//   try {
//     await DatCocBUS.reject(parseInt(req.params.id), req.body.ghi_chu || '');
//     res.json({ success: true, message: 'Đã từ chối. Khách hàng cần thanh toán lại.' });
//   } catch (err: any) {
//     res.status(400).json({ success: false, error: err.message });
//   }
// });
router.post('/:id/members', async (req: Request, res: Response) => {
  try {
    ...
    await DatCocBUS.saveGroupMembers(maHD, members);
