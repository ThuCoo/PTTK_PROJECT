...
exports.getSelectedRooms = getSelectedRooms;
exports.checkCustomerConditions = checkCustomerConditions;
 ...
 */
async function getById(maPhieu) {
    const result = await (0, db_1.query)(`
    SELECT 
      pdk.ma_phieu_dk,
      pdk.ma_khach_hang,
 ...
 */
async function getSelectedRooms(maPhieuDangKy) {
    const result = await (0, db_1.query)(`
    SELECT 
      p.ma_phong,
      p.loai_phong,
 ...
 */
async function checkCustomerConditions(maKhachHang, maPhong) {
    // Get room's required conditions
    const roomConditions = await (0, db_1.query)(`
    SELECT 
    ...
    const customerConditions = await (0, db_1.query)(`
    SELECT 
      khdk.dieu_kien_id,
      khdk.trang_thai,
      khdk.ghi_chu
    ...
    customerConditions.rows.forEach((c) => {
        conditionMap.set(c.dieu_kien_id, {
            trang_thai: c.trang_thai,
            ghi_chu: c.ghi_chu,
        });
    });
    const results = roomConditions.rows.map((rc) => ({
        id: rc.id,
    ...
    return {
        ...
        all_passed: results.every((c) => c.trang_thai_khach === "Đã duyệt"),
    };
}
/**
 * Get room availability and status
 */
async function getRoomStatus(maPhong) {
    const result = await (0, db_1.query)(`
    SELECT 
      ma_phong,
      loai_phong,
 ...
 */
async function confirmReview(maPhieuDangKy, maPhong, ghiChu) {
    const result = await (0, db_1.query)(`
 ...
 */
async function getCustomer(maKhachHang) {
    const result = await (0, db_1.query)(`
    SELECT 
      ma_khach_hang,
      ho_ten,
 ...
 */
async function isRoomAvailable(maPhong) {
    const result = await (0, db_1.query)(`
    SELECT trang_thai
    FROM phong
    WHERE ma_phong = $1
 ...
 */
async function markAsInvalid(maPhieuDangKy, reason) {
    const result = await (0, db_1.query)(`
    UPDATE phieu_dang_ky
    SET 
